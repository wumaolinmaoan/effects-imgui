...
export const IMGUI_VERSION: string = "1.92.7"; // bind.IMGUI_VERSION;
export { IMGUI_VERSION_NUM as VERSION_NUM }
export const IMGUI_VERSION_NUM: number = 19270; // bind.IMGUI_VERSION_NUM;
...
export enum ImGuiWindowFlags {
    ...
    NoBackground           = 1 << 7,   // Disable drawing background color (WindowBg, etc.) and outside border
    ...
    UnsavedDocument        = 1 << 18,  // Display a dot next to the title. When used in a tab/docking context, tab is selected on closure and closure is deferred by one frame to allow code to undo it without flicker.
    ...
    // [Deprecated]
    AlwaysUseWindowPadding = 1 << 30,  // [Removed in 1.90] Use ImGuiChildFlags.AlwaysUseWindowPadding
    NavFlattened           = 1 << 31,  // [Removed] Use ImGuiChildFlags.NavFlattened
}

// Flags for ImGui::BeginChild()
export { ImGuiChildFlags as ChildFlags };
export enum ImGuiChildFlags {
    None                    = 0,
    Borders                 = 1 << 0,
...
export enum ImGuiInputTextFlags {
    ...
    EnterReturnsTrue    = 1 << 6,   // Return 'true' when Enter is pressed (as opposed to every time the value was modified)
    ...
    CtrlEnterForNewLine = 1 << 8,   // In multi-line mode, unfocus with Enter, add new line with Ctrl+Enter (default is opposite: unfocus with Ctrl+Enter, add line with Enter).
    ...
    ParseEmptyRefVal    = 1 << 13,  // InputFloat(), InputInt(), InputScalar() etc.: parse empty string as zero value.
    DisplayEmptyRefVal  = 1 << 14,  // InputFloat(), InputInt(), InputScalar() etc.: when value is zero, do not display it.
    NoHorizontalScroll  = 1 << 15,  // Disable following the cursor horizontally
    NoUndoRedo          = 1 << 16,  // Disable undo/redo.
    ElideLeft           = 1 << 17,  // When text doesn't fit, elide left side so right side has priority.
    ...
    CallbackAlways      = 1 << 20,  // Callback on each iteration.
    CallbackCharFilter  = 1 << 21,  // Callback on character inputs to replace or discard them.
    CallbackResize      = 1 << 22,  // Callback on buffer capacity changes request.
    CallbackEdit        = 1 << 23,  // Callback on any edit.
    WordWrap            = 1 << 24,  // Multi-line: enable word-wrap.

    // [Deprecated]
    AlwaysInsertMode    = 1 << 11,  // [renamed in 1.87] Use AlwaysOverwrite instead
}
...
export enum ImGuiTreeNodeFlags {
    ...
    SpanAllColumns       = 1 << 14,  // Frame will span all columns of its container table (text will still fit in current column)
    LabelSpanAllColumns  = 1 << 15,  // Label will span all columns of its container table
    NavLeftJumpsToParent = 1 << 17,  // (WIP) Nav: left direction may move to this TreeNode() from any of its child
    CollapsingHeader     = Framed | NoTreePushOnOpen | NoAutoOpenOnLog,

    // [Experimental] Tree hierarchy lines
    DrawLinesNone        = 1 << 18,  // [EXPERIMENTAL] Draw no lines
    DrawLinesFull        = 1 << 19,  // [EXPERIMENTAL] Draw full hierarchy lines
    DrawLinesToNodes      = 1 << 20,  // [EXPERIMENTAL] Draw hierarchy lines only to nodes

    // [Deprecated]
    AllowItemOverlap     = AllowOverlap,         // [renamed in 1.89.7]
    NavLeftJumpsBackHere = NavLeftJumpsToParent,  // [renamed in 1.92.0]
    SpanTextWidth        = SpanLabelWidth,        // [renamed in 1.90.7]
}

export { ImGuiPopupFlags as PopupFlags };
export enum ImGuiPopupFlags {
    None                    = 0,
    MouseButtonLeft         = 1 << 2,   // For BeginPopupContext*(): open on Left Mouse release.
    MouseButtonRight        = 2 << 2,   // For BeginPopupContext*(): open on Right Mouse release. (default)
    MouseButtonMiddle       = 3 << 2,   // For BeginPopupContext*(): open on Middle Mouse release.
    MouseButtonMask_        = 0x0C,
    NoReopen                = 1 << 5,   // For OpenPopup*(): don't reopen same popup if already open
...
export enum ImGuiSelectableFlags {
    None               = 0,
    NoAutoClosePopups  = 1 << 0,   // Clicking this don't close parent popup window
    ...
    SelectOnNav        = 1 << 6,   // Automatically select item when navigating (useful for combo)

    // [Deprecated]
    DontClosePopups    = NoAutoClosePopups,  // [renamed in 1.91.0]
    AllowItemOverlap   = AllowOverlap,       // [renamed in 1.89.7]
}

export { ImGuiMultiSelectFlags as MultiSelectFlags };
export enum ImGuiMultiSelectFlags {
    None                          = 0,
    SingleSelect                  = 1 << 0,
    NoSelectAll                   = 1 << 1,
    NoRangeSelect                 = 1 << 2,
    ...
    SelectOnMask_                 = SelectOnAuto | SelectOnClickAlways | SelectOnClickRelease,

    // [Deprecated]
    SelectOnClick                 = SelectOnAuto,
}

export { ImGuiSelectionRequestType as SelectionRequestType };
export enum ImGuiSelectionRequestType {
    None                          = 0,
    SetAll                        = 1,
    SetRange                      = 2,
}
...
export enum ImGuiTabBarFlags {
    ...
    NoTabListScrollingButtons      = 1 << 4,   // Disable scrolling buttons
    NoTooltip                      = 1 << 5,   // Disable tooltips when hovering a tab
    DrawSelectedOverline           = 1 << 6,   // Draw selected overline markers on selected tabs
    FittingPolicyMixed             = 1 << 7,   // Shrink tabs first, then add scroll buttons
    FittingPolicyShrink            = 1 << 8,   // Shrink tabs to fit
    FittingPolicyScroll            = 1 << 9,   // Add scroll buttons when tabs don't fit
    FittingPolicyMask_             = FittingPolicyMixed | FittingPolicyShrink | FittingPolicyScroll,
    FittingPolicyDefault_          = FittingPolicyMixed,

    // [Deprecated]
    FittingPolicyResizeDown        = FittingPolicyShrink, // [renamed in 1.92.2]
};
...
export enum ImGuiTabItemFlags {
    ...
    NoAssumedClosure              = 1 << 8,   // Tab is selected when trying to close + closure is not immediately assumed (will wait for user to stop submitting the tab)
}
...
export enum ImGuiTableFlags {
    ...
    HighlightHoveredColumn     = 1 << 28,  // Highlight column headers when hovered (may evolve into a per-column flag)
...
export enum ImGuiTableColumnFlags {
    ...
    None                  = 0,
    Disabled              = 1 << 0,   // Overriding/master disable flag: hide column, won't show in context menu.
    DefaultHide           = 1 << 1,   // Default as a hidden/disabled column.
    DefaultSort           = 1 << 2,   // Default as a sorting column.
    WidthStretch          = 1 << 3,   // Column will stretch.
    WidthFixed            = 1 << 4,   // Column will not stretch.
    NoResize              = 1 << 5,   // Disable manual resizing.
    NoReorder             = 1 << 6,   // Disable manual reordering this column.
    NoHide                = 1 << 7,   // Disable ability to hide/disable this column.
    NoClip                = 1 << 8,   // Disable clipping for this column.
    NoSort                = 1 << 9,   // Disable ability to sort on this field.
    ...
    NoHeaderLabel         = 1 << 12,  // TableHeadersRow() will submit an empty label for this column.
    ...
    IndentDisable         = 1 << 17,  // Ignore current Indent value when entering cell (default for columns > 0).
    AngledHeader          = 1 << 18,  // TableHeadersRow() will submit an angled header row for this column.

    // Output status flags, read-only via TableGetColumnFlags()
    IsEnabled             = 1 << 24,  // Status: is enabled == not hidden by user/api.
    ...
    NoDirectResize_       = 1 << 30,  // [Internal] Disable user resizing this column directly.
}
...
export enum ImGuiFocusedFlags {
    ...
    AnyWindow                     = 1 << 2,   // Return true if any window is focused
    NoPopupHierarchy              = 1 << 3,   // Do not consider popup hierarchy
    DockHierarchy                 = 1 << 4,   // Consider docking hierarchy
...
export enum ImGuiHoveredFlags {
    ...
    AllowWhenOverlappedByItem     = 1 << 8,   // Return true even if the item uses AllowOverlap mode and is overlapped by another hoverable item.
    AllowWhenOverlappedByWindow   = 1 << 9,   // Return true even if the position is obstructed or overlapped by another window.
    AllowWhenDisabled             = 1 << 10,  // Return true even if the item is disabled
    NoNavOverride                 = 1 << 11,  // Disable using keyboard/gamepad navigation state when active, always query mouse
    ...
    Stationary                    = 1 << 13,  // Require mouse to be stationary for style.HoverStationaryDelay
    DelayNone                     = 1 << 14,  // Return true immediately (default)
    DelayShort                    = 1 << 15,  // Return true after style.HoverDelayShort elapsed
    DelayNormal                   = 1 << 16,  // Return true after style.HoverDelayNormal elapsed
    NoSharedDelay                 = 1 << 17,  // Disable shared delay system
}

export { ImGuiInputFlags as InputFlags };
export enum ImGuiInputFlags {
    None                          = 0,
    Repeat                        = 1 << 0,
    RouteActive                   = 1 << 10,
    RouteFocused                  = 1 << 11,
...
export enum ImGuiViewportFlags {
    None                     = 0,
    IsPlatformWindow         = 1 << 0,   // Represents a platform window
    IsPlatformMonitor        = 1 << 1,   // Represents a platform monitor (unused yet)
    OwnedByApp              = 1 << 2,   // Platform Window: Was created/managed by the user application
    NoDecoration             = 1 << 3,   // Platform Window: Disable platform decorations: title bar, borders, etc.
    NoTaskBarIcon            = 1 << 4,   // Platform Window: Disable platform task bar icon
    ...
    NoRendererClear          = 1 << 8,   // Platform Window: Renderer doesn't need to clear the framebuffer ahead.
    NoAutoMerge              = 1 << 9,   // Platform Window: Avoid merging this window into another host window.
    TopMost                  = 1 << 10,  // Platform Window: Display on top (for tooltips only).
    CanHostOtherWindows      = 1 << 11,  // Main viewport: can host multiple imgui windows.
    IsMinimized              = 1 << 12,  // Platform Window: Window is minimized, can skip render.
    IsFocused                = 1 << 13,  // Platform Window: Window is focused.

    // [Deprecated]
    Minimized                = 1 << 12,  // [renamed] Use IsMinimized
}
...
export enum ImGuiDockNodeFlags {
    None                         = 0,
    KeepAliveOnly                = 1 << 0,   // Don't display the dockspace node but keep it alive.
    NoDockingOverCentralNode     = 1 << 2,   // Disable docking over the Central Node, which will be always kept empty.
    PassthruCentralNode          = 1 << 3,   // Enable passthru dockspace: transparent background when central node is empty.
    NoDockingSplit               = 1 << 4,   // Disable splitting the node into smaller nodes.
    NoResize                     = 1 << 5,   // Disable resizing node using the splitter/separators.
...
export enum ImGuiDragDropFlags {
    ...
    SourceNoDisableHover         = 1 << 1,   // Keep IsItemHovered() returning true while dragging.
    SourceNoHoldToOpenOthers     = 1 << 2,   // Disable the behavior that allows to open tree nodes by holding over them while dragging.
    SourceAllowNullID            = 1 << 3,   // Allow items such as Text(), Image() that have no unique identifier to be used as drag source.
    SourceExtern                 = 1 << 4,   // External source (from outside of Dear ImGui).
    PayloadAutoExpire            = 1 << 5,   // Automatically expire the payload if the source cease to be submitted.
    PayloadNoCrossContext        = 1 << 6,   // Hint: Don't copy payload across Dear ImGui contexts.
    PayloadNoCrossProcess        = 1 << 7,   // Hint: Don't copy payload across OS processes.
    ...
    AcceptDrawAsHovered          = 1 << 13,  // Make the target item render as hovered.
    AcceptPeekOnly               = AcceptBeforeDelivery | AcceptNoDrawDefaultRect,  // For peeking ahead and inspecting the payload before delivery.

    // [Deprecated]
    SourceAutoExpirePayload      = PayloadAutoExpire,  // [renamed in 1.90.9]
}
...
export enum ImGuiKey {
    ...
    Equal = 602,        // =
    LeftBracket = 603,  // [
    Backslash = 604,    // (backslash)
    RightBracket = 605, // ]
    GraveAccent = 606,  // ` (backtick)
    CapsLock = 607,
    ScrollLock = 608,
    NumLock = 609,
    PrintScreen = 610,
    ...
    // Mouse Buttons
    MouseLeft = 656, MouseRight = 657, MouseMiddle = 658,
    MouseX1 = 659, MouseX2 = 660, MouseWheelX = 661, MouseWheelY = 662,

    // [Internal] Reserved for mod storage
    ReservedForModCtrl = 663, ReservedForModShift = 664,
    ReservedForModAlt = 665, ReservedForModSuper = 666,

    NamedKey_END = 667,
    NamedKey_COUNT = 155, // NamedKey_END - NamedKey_BEGIN
    ...
    Mod_Shift = 1 << 13,  // 8192
    Mod_Alt = 1 << 14,    // 16384
    Mod_Super = 1 << 15,  // 32768
    Mod_Mask_ = 0xF000,   // 61440
...
// [Obsolete] Renamed to ImGuiMod in v1.89
...
export enum ImGuiConfigFlags
{
    ...
    NoMouseCursorChange  = 1 << 5,   // Instruct backend to not alter mouse cursor shape and visibility.
    NoKeyboard           = 1 << 6,   // Instruct dear imgui to disable keyboard inputs and interactions.

    // [BETA] Docking
    DockingEnable        = 1 << 7,   // Docking enable flags.

    // [BETA] Viewports
    ViewportsEnable      = 1 << 10,  // Viewport enable flags (require both ImGuiBackendFlags_PlatformHasViewports + ImGuiBackendFlags_RendererHasViewports)
...
export enum ImGuiCol {
    ...
    TabDimmedSelectedOverline,     // 40 Tab horizontal overline, when tab-bar is unfocused & tab is selected
    ...
    TableBorderStrong,             // 48 Table outer and header borders
    TableBorderLight,              // 49 Table inner borders
    TableRowBg,                    // 50 Table row background (even rows)
    TableRowBgAlt,                 // 51 Table row background (odd rows)
    TextLink,                      // 52 Hyperlink color
    TextSelectedBg,                // 53
    TreeLines,                     // 54 Tree node hierarchy outlines
    DragDropTarget,                // 55 Rectangle highlighting a drop target
    DragDropTargetBg,              // 56 Drop target background highlight
    UnsavedMarker,                 // 57 Unsaved document marker
    NavCursor,                     // 58 Gamepad/keyboard: current highlighted item
    ...
    // [Deprecated]
    TabActive             = TabSelected,             // [renamed in 1.90.9]
    TabUnfocused          = TabDimmed,               // [renamed in 1.90.9]
    TabUnfocusedActive    = TabDimmedSelected,       // [renamed in 1.90.9]
    NavHighlight          = NavCursor,               // [renamed in 1.91.4]
}
...
export enum ImGuiBackendFlags {
    None                       = 0,
    HasGamepad                 = 1 << 0,   // Backend supports gamepad and currently has one connected.
    HasMouseCursors            = 1 << 1,   // Backend supports honoring GetMouseCursor() value to change the OS cursor shape.
    HasSetMousePos             = 1 << 2,   // Backend supports io.WantSetMousePos requests to reposition the OS mouse position.
    ...
    RendererHasTextures        = 1 << 4,   // Backend Renderer supports ImTextureData* based texture operations.

    // [BETA] Viewports
    PlatformHasViewports       = 1 << 10,  // Backend Platform supports multiple viewports.
    RendererHasViewports       = 1 << 11,  // Backend Renderer supports multiple viewports.
    HasMouseHoveredViewport    = 1 << 12,  // Backend Platform supports calling io.AddMouseViewportEvent() with the viewport under the mouse.
}
...
export enum ImGuiButtonFlags {
    ...
    EnableNav              = 1 << 3,   // InvisibleButton(): enable navigation (default behavior for regular buttons)
    AllowOverlap           = 1 << 12,  // Allow subsequent overlapping items to take priority
...
export enum ImGuiColorEditFlags {
    None               = 0,
    NoAlpha            = 1 << 1,   // ColorEdit, ColorPicker, ColorButton: ignore Alpha component (will only read/write 3 components).
    NoPicker           = 1 << 2,   // ColorEdit: disable picker when clicking on color square.
    NoOptions          = 1 << 3,   // ColorEdit: disable toggling options menu when right-clicking on inputs/small preview.
    NoSmallPreview     = 1 << 4,   // ColorEdit, ColorPicker: disable color square preview next to the inputs.
    NoInputs           = 1 << 5,   // ColorEdit, ColorPicker: disable inputs sliders/text widgets.
    NoTooltip          = 1 << 6,   // ColorEdit, ColorPicker, ColorButton: disable tooltip when hovering the preview.
    NoLabel            = 1 << 7,   // ColorEdit, ColorPicker: disable display of inline text label.
    NoSidePreview      = 1 << 8,   // ColorPicker: disable bigger color preview on right side of the picker.
    ...
    NoColorMarkers     = 1 << 11,  // Disable R/G/B/A markers on sliders.
    AlphaOpaque        = 1 << 12,  // ColorPreview: force opaque alpha preview.
    AlphaNoBg          = 1 << 13,  // ColorPreview: disable checkerboard background.
    AlphaPreviewHalf   = 1 << 14,  // ColorPreview: half transparent / half checkerboard.
    // User Options (right-click on widget to change some of them).
    AlphaBar           = 1 << 18,  // ColorEdit, ColorPicker: show vertical alpha bar/gradient in picker.
    HDR                = 1 << 19,  // (WIP) ColorEdit: Currently only disable 0.0f..1.0f limits in RGBA edition.
    DisplayRGB         = 1 << 20,  // [Display] ColorEdit: override display type among RGB/HSV/Hex.
    ...
    Float              = 1 << 24,  // [DataType] ColorEdit, ColorPicker, ColorButton: _display_ values formatted as 0.0f..1.0f.
    ...
    // [Internal] Masks
    _AlphaMask         = AlphaOpaque | AlphaNoBg | AlphaPreviewHalf,
    ...
    // [Deprecated]
    AlphaPreview       = 0,        // [Removed in 1.91.8]
}
...
export enum ImGuiSliderFlags {
    None                   = 0,
    Logarithmic            = 1 << 5,       // Make the widget logarithmic (linear otherwise).
    NoRoundToFormat        = 1 << 6,       // Disable rounding underlying value to match precision of the display format string.
    NoInput                = 1 << 7,       // Disable CTRL+Click or Enter key allowing to input text directly into the widget.
    WrapAround             = 1 << 8,       // Enable wrapping around from max to min and vice-versa.
    ClampOnInput           = 1 << 9,       // Clamp value to min/max bounds when input manually with CTRL+Click.
    ClampZeroRange         = 1 << 10,      // Clamp even if min==max==0.0f (otherwise DragXXX functions don't clamp).
    NoSpeedTweaks          = 1 << 11,      // Disable keyboard modifiers altering drag/slider speed.
    ColorMarkers           = 1 << 12,      // Display R/G/B/A markers on color sliders.
    AlwaysClamp            = ClampOnInput | ClampZeroRange,  // Composite of ClampOnInput + ClampZeroRange
    InvalidMask_           = 0x7000000F,   // [Internal]
}
...
export enum ImGuiMouseCursor {
    ...
    Wait,              // When waiting for processing/loading
    Progress,          // When processing but still interactive
...
export enum ImDrawFlags
{
    ...
    RoundCornersTopLeft     = 1 << 4,   // AddRect(), AddRectFilled(), PathRect(): enable rounding top-left corner only
    RoundCornersTopRight    = 1 << 5,   // AddRect(), AddRectFilled(), PathRect(): enable rounding top-right corner only
    RoundCornersBottomLeft  = 1 << 6,   // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-left corner only
    RoundCornersBottomRight = 1 << 7,   // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-right corner only
    RoundCornersNone        = 1 << 8,   // AddRect(), AddRectFilled(), PathRect(): disable rounding on all corners (when radius > 0.0f)
    ...
    RoundCornersDefault_    = RoundCornersAll,       // Default to all corners if none of the RoundCornersXX flags are specified.
...
{
    constructor(public readonly native: Bind.reference_ImGuiStorage = new bind.ImGuiStorage()) {}

    public delete(): void {
        if (this.native instanceof bind.ImGuiStorage) {
            (this.native as Bind.ImGuiStorage).delete();
        }
    }

    public Clear(): void { this.native.Clear(); }
    public GetInt(key: ImGuiID, default_val: number = 0): number { return this.native.GetInt(key, default_val); }
    public SetInt(key: ImGuiID, val: number): void { this.native.SetInt(key, val); }
    public GetBool(key: ImGuiID, default_val: boolean = false): boolean { return this.native.GetBool(key, default_val); }
    public SetBool(key: ImGuiID, val: boolean): void { this.native.SetBool(key, val); }
    public GetFloat(key: ImGuiID, default_val: number = 0.0): number { return this.native.GetFloat(key, default_val); }
    public SetFloat(key: ImGuiID, val: number): void { this.native.SetFloat(key, val); }
    public GetVoidPtr(key: ImGuiID): number | null { return this.native.GetVoidPtr(key); }
    public SetVoidPtr(key: ImGuiID, val: number | null): void { this.native.SetVoidPtr(key, val); }
    public BuildSortByKey(): void { this.native.BuildSortByKey(); }
    public SetAllInt(val: number): void { this.native.SetAllInt(val); }
}
...
export class ImGuiWindowClass implements Bind.interface_ImGuiWindowClass {
    constructor(public ClassId: number = 0,
                public ParentViewportId: number = 0,
                public ViewportFlagsOverrideSet: number = 0,
                public ViewportFlagsOverrideClear: number = 0,
...
export class ImGuiListClipper
{
    ...
    public IncludeItemByIndex(item_index: number): void {
        this.native.IncludeItemByIndex(item_index);
    }
    public IncludeItemsByIndex(item_begin: number, item_end: number): void {
        this.native.IncludeItemsByIndex(item_begin, item_end);
    }
    public SeekCursorForItem(item_index: number): void {
        this.native.SeekCursorForItem(item_index);
    }
    public get UserIndex(): number { return this.native.UserIndex; }
    public set UserIndex(value: number) { this.native.UserIndex = value; }
}
...
export class ImGuiViewport
{
    ...
    GetWorkCenter(): Bind.interface_ImVec2         { return new ImVec2(this.native.WorkPos.x + this.native.WorkSize.x * 0.5, this.native.WorkPos.y + this.native.WorkSize.y * 0.5); }
    GetWorkPos(): Bind.interface_ImVec2            { return new ImVec2(this.native.WorkPos.x, this.native.WorkPos.y); }
    GetWorkSize(): Bind.interface_ImVec2           { return new ImVec2(this.native.WorkSize.x, this.native.WorkSize.y); }
...
{
    constructor(public readonly native: Bind.interface_ImGuiPlatformMonitor) {}

    get MainPos(): Readonly<Bind.interface_ImVec2> { return this.native.MainPos; }
    get MainSize(): Readonly<Bind.interface_ImVec2> { return this.native.MainSize; }
    get WorkPos(): Readonly<Bind.interface_ImVec2> { return this.native.WorkPos; }
    get WorkSize(): Readonly<Bind.interface_ImVec2> { return this.native.WorkSize; }
    get DpiScale(): number { return this.native.DpiScale; }
    set DpiScale(value: number) { this.native.DpiScale = value; }
    get PlatformHandle(): number | null { return this.native.PlatformHandle; }
    set PlatformHandle(value: number | null) { this.native.PlatformHandle = value; }
}

export { ImGuiPlatformIO as PlatformIO }
export class ImGuiPlatformIO
{
    constructor(public readonly native: Bind.reference_ImGuiPlatformIO) {}

    get Monitors(): Readonly<ImGuiPlatformMonitor[]> {
        const monitors: ImGuiPlatformMonitor[] = [];
        this.native.IterateMonitors((monitor: Bind.interface_ImGuiPlatformMonitor): void => {
    ...
    get Viewports(): Readonly<ImGuiViewport[]> {
        const viewports: ImGuiViewport[] = [];
        this.native.IterateViewports((viewport: Bind.reference_ImGuiViewport): void => {
            viewports.push(new ImGuiViewport(viewport));
        });
        return viewports;
    }

    get Platform_LocaleDecimalPoint(): number { return this.native.Platform_LocaleDecimalPoint; }
    set Platform_LocaleDecimalPoint(value: number) { this.native.Platform_LocaleDecimalPoint = value; }
    get Renderer_TextureMaxWidth(): number { return this.native.Renderer_TextureMaxWidth; }
    set Renderer_TextureMaxWidth(value: number) { this.native.Renderer_TextureMaxWidth = value; }
    get Renderer_TextureMaxHeight(): number { return this.native.Renderer_TextureMaxHeight; }
    set Renderer_TextureMaxHeight(value: number) { this.native.Renderer_TextureMaxHeight = value; }
    get Renderer_RenderState(): number | null { return this.native.Renderer_RenderState; }

    public ClearPlatformHandlers(): void { this.native.ClearPlatformHandlers(); }
    public ClearRendererHandlers(): void { this.native.ClearRendererHandlers(); }
}
...
{
    constructor(public readonly native: Bind.reference_ImGuiMultiSelectIO) {}

    get Requests(): Readonly<ImGuiSelectionRequest[]> {
        const raw = this.native._getRequests();
        return raw.map((request: Bind.interface_ImGuiSelectionRequest) => ({
            Type: request.Type as ImGuiSelectionRequestType,
    ...
    get RangeSrcItem(): ImGuiSelectionUserData { return this.native.RangeSrcItem; }
    get NavIdItem(): ImGuiSelectionUserData { return this.native.NavIdItem; }
    get NavIdSelected(): boolean { return this.native.NavIdSelected; }
    get RangeSrcReset(): boolean { return this.native.RangeSrcReset; }
    set RangeSrcReset(value: boolean) { this.native.RangeSrcReset = value; }
    get ItemsCount(): number { return this.native.ItemsCount; }
}

export { ImGuiSelectionBasicStorage as SelectionBasicStorage }
export class ImGuiSelectionBasicStorage
{
    public PreserveOrder: boolean = false;
    public UserData: any = null;
    public AdapterIndexToStorageId: (self: ImGuiSelectionBasicStorage, idx: number) => ImGuiID = (_self, idx) => idx;
    public _SelectionOrder: number = 0;
    private _storage: Map<ImGuiID, number> = new Map();

    get Size(): number { return this._storage.size; }

    public ApplyRequests(ms_io: ImGuiMultiSelectIO | null): void {
        if (ms_io === null) {
            return;
        }
    ...
    public Contains(id: ImGuiID): boolean { return this._storage.has(id); }
    public Clear(): void {
        this._storage.clear();
        this._SelectionOrder = 0;
    }
    public Swap(r: ImGuiSelectionBasicStorage): void {
    ...
    public SetItemSelected(id: ImGuiID, selected: boolean): void {
        if (selected) {
    ...
    public GetNextSelectedItem(opaque_it: Bind.ImScalar<number | null>, out_id: Bind.ImScalar<ImGuiID>): boolean {
    ...
    public GetStorageIdFromIndex(idx: number): ImGuiID {
        return this.AdapterIndexToStorageId(this, idx);
    }

    private getOrderedIds(): ImGuiID[] {
...
{
    public UserData: any = null;
    public AdapterSetItemSelected: (self: ImGuiSelectionExternalStorage, idx: number, selected: boolean) => void = () => {};
...
export class ImDrawListSplitter
{
    ...
    public ClearFreeMemory(): void { this.Clear(); }
    public Split(draw_list: ImDrawList, count: number): void {
        draw_list.ChannelsSplit(count);
        this._Current = 0;
        this._Count = count;
    }
    public Merge(draw_list: ImDrawList): void {
        draw_list.ChannelsMerge();
        this.Clear();
    }
    public SetCurrentChannel(draw_list: ImDrawList, channel_idx: number): void {
...
export class ImDrawList
{
    ...
    public AddConcavePolyFilled(points: Array<Readonly<Bind.interface_ImVec2>>, num_points: number, col: Bind.ImU32): void {
    ...
    public AddEllipse(center: Readonly<Bind.interface_ImVec2>, radius: Readonly<Bind.interface_ImVec2>, col: Bind.ImU32, rot: number = 0.0, num_segments: number = 0, thickness: number = 1.0): void {
    ...
    public AddEllipseFilled(center: Readonly<Bind.interface_ImVec2>, radius: Readonly<Bind.interface_ImVec2>, col: Bind.ImU32, rot: number = 0.0, num_segments: number = 0): void {
    ...
    public PathFillConcave(col: Bind.ImU32): void { this.native.PathFillConcave(col); }
    ...
    public PathStroke(col: Bind.ImU32, flags: ImDrawFlags = 0, thickness: number = 1.0): void { this.native.PathStroke(col, flags, thickness); }
    ...
    public PathEllipticalArcTo(center: Readonly<Bind.interface_ImVec2>, radius: Readonly<Bind.interface_ImVec2>, rot: number, a_min: number, a_max: number, num_segments: number = 0): void { this.native.PathEllipticalArcTo(center, radius, rot, a_min, a_max, num_segments); }
...
export class script_ImFontConfig implements Bind.interface_ImFontConfig
{
    ...
    FontLoaderFlags: number = 0;
    RasterizerMultiply: number = 1.0;
    RasterizerDensity: number = 1.0;
...
export class ImFontConfig {
    ...
    get FontLoaderFlags(): number { return this.internal.FontLoaderFlags; }
    get RasterizerMultiply(): number { return this.internal.RasterizerMultiply; }
    get RasterizerDensity(): number { return this.internal.RasterizerDensity; }
...
{
    private _usedChars: Set<number> = new Set();

    public Clear(): void { this._usedChars.clear(); }
    public GetBit(n: number): boolean { return this._usedChars.has(n); }
    public SetBit(n: number): void { this._usedChars.add(n); }
    public AddChar(c: Bind.ImWchar): void { this.SetBit(c); }
    public AddText(text: string, text_end: number | null = null): void {
    ...
    public AddRanges(ranges: number | ArrayLike<number>): void {
    ...
    public BuildRangesArray(): Uint16Array {
    ...
    public BuildRanges(): number { return GlyphRangeAlloc(this.BuildRangesArray()); }
}
...
{
    constructor(public readonly native: Bind.interface_ImFontAtlasRect) {}

    get x(): number { return this.native.x; }
    get y(): number { return this.native.y; }
    get w(): number { return this.native.w; }
    get h(): number { return this.native.h; }
    get uv0(): Readonly<Bind.interface_ImVec2> { return this.native.uv0; }
    get uv1(): Readonly<Bind.interface_ImVec2> { return this.native.uv1; }
}

function unwrapFontConfig(font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null): Bind.interface_ImFontConfig | null {
    return font_cfg instanceof ImFontConfig ? font_cfg.internal : font_cfg;
}

function fontBinaryToUint8Array(data: ArrayBuffer | Uint8Array): Uint8Array {
...
export class ImFontAtlas
{
    ...
    public AddFont(font_cfg: ImFontConfig | Bind.interface_ImFontConfig): ImFont {
        return new ImFont(this.native.AddFont(unwrapFontConfig(font_cfg)!));
    }
    // IMGUI_API ImFont*           AddFontDefault(const ImFontConfig* font_cfg = NULL);
    public AddFontDefault(font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null = null): ImFont {
        return new ImFont(this.native.AddFontDefault(unwrapFontConfig(font_cfg)));
    }
    public AddFontDefaultVector(font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null = null): ImFont {
        return new ImFont(this.native.AddFontDefaultVector(unwrapFontConfig(font_cfg)));
    }
    public AddFontDefaultBitmap(font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null = null): ImFont {
        return new ImFont(this.native.AddFontDefaultBitmap(unwrapFontConfig(font_cfg)));
    }
    // IMGUI_API ImFont*           AddFontFromFileTTF(const char* filename, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);
    public AddFontFromFileTTF(filename: string, size_pixels: number, font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null = null, glyph_ranges: number | null = null): ImFont {
    ...
    public AddFontFromMemoryTTF(data: ArrayBuffer | Uint8Array, size_pixels: number, font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null = null, glyph_ranges: number | null = null): ImFont {
    ...
    public AddFontFromMemoryCompressedTTF(data: ArrayBuffer | Uint8Array, size_pixels: number, font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null = null, glyph_ranges: number | null = null): ImFont {
    ...
    public AddFontFromMemoryCompressedBase85TTF(data: string, size_pixels: number, font_cfg: ImFontConfig | Bind.interface_ImFontConfig | null = null, glyph_ranges: number | null = null): ImFont {
    ...
    public RemoveFont(font: ImFont | null): void { this.native.RemoveFont(font ? font.native : null); }
    ...
    public Clear(): void { this.native.Clear(); }
    public CompactCache(): void { this.native.CompactCache(); }
    ...
    GetGlyphRangesGreek(): number { return this.native.GetGlyphRangesGreek(); }
    ...
    public AddCustomRect(width: number, height: number): ImFontAtlasRectId { return this.native.AddCustomRect(width, height); }
    public RemoveCustomRect(id: ImFontAtlasRectId): void { this.native.RemoveCustomRect(id); }
    public GetCustomRect(id: ImFontAtlasRectId): ImFontAtlasRect | null {
        const rect = this.native.GetCustomRect(id);
        return rect === null ? null : new ImFontAtlasRect(rect);
    }
    public AddCustomRectFontGlyph(font: ImFont | null, codepoint: Bind.ImWchar, w: number, h: number, advance_x: number, offset: Readonly<Bind.interface_ImVec2> = ImVec2.ZERO): ImFontAtlasRectId {
        return this.native.AddCustomRectFontGlyph(font ? font.native : null, codepoint, w, h, advance_x, offset);
    }
    public AddCustomRectFontGlyphForSize(font: ImFont | null, font_size: number, codepoint: Bind.ImWchar, w: number, h: number, advance_x: number, offset: Readonly<Bind.interface_ImVec2> = ImVec2.ZERO): ImFontAtlasRectId {
    ...
    get TexMinWidth(): number { return this.native.TexMinWidth; }
    set TexMinWidth(value: number) { this.native.TexMinWidth = value; }
    get TexMinHeight(): number { return this.native.TexMinHeight; }
    set TexMinHeight(value: number) { this.native.TexMinHeight = value; }
    get TexMaxWidth(): number { return this.native.TexMaxWidth; }
    set TexMaxWidth(value: number) { this.native.TexMaxWidth = value; }
    get TexMaxHeight(): number { return this.native.TexMaxHeight; }
    set TexMaxHeight(value: number) { this.native.TexMaxHeight = value; }
...
{
    constructor(public readonly native: Bind.reference_ImFontBaked) {}

    get Glyphs(): ImVector<ImFontGlyph> {
        const glyphs = new ImVector<ImFontGlyph>();
        this.native.IterateGlyphs((glyph: Bind.interface_ImFontGlyph): void => {
    ...
    get RasterizerDensity(): number { return this.native.RasterizerDensity; }
    get FallbackGlyphIndex(): number { return this.native.FallbackGlyphIndex; }
    ...
    public FindGlyphNoFallback(c: number): Readonly<ImFontGlyph> | null {
        const glyph = this.native.FindGlyphNoFallback(c);
        return glyph === null ? null : new ImFontGlyph(glyph);
    }
    public GetCharAdvance(c: number): number { return this.native.GetCharAdvance(c); }
    public IsGlyphLoaded(c: number): boolean { return this.native.IsGlyphLoaded(c); }
}
...
export class ImFont
{
    ...
    get FontSize(): number { return this.native.LegacySize; }
    // ImGuiID                     FontId;             // Unique identifier for the font
    get FontId(): number { return this.native.FontId; }
    ...
    public GetCharAdvance(c: number): number { return this.native.GetCharAdvance(c); }
    public GetFontBaked(font_size: number, density: number = -1.0): ImFontBaked | null {
    ...
    public RenderText(draw_list: ImDrawList, size: number, pos: Readonly<Bind.interface_ImVec2>, col: Bind.ImU32, clip_rect: Readonly<Bind.interface_ImVec4>, text_begin: string, text_end: number | null = null, wrap_width: number = 0.0, flags: ImDrawFlags = 0): void {
...
class script_ImGuiStyle implements Bind.interface_ImGuiStyle {
    public FontSizeBase: number = 0.0;
    ...
    public WindowBorderHoverPadding: number = 0.0;
    ...
    public ScrollbarPadding: number = 0.0;
    public GrabMinSize: number = 10.0;
    public GrabRounding: number = 0.0;
    public LogSliderDeadzone: number = 4.0;
    public ImageRounding: number = 0.0;
    public ImageBorderSize: number = 0.0;
    public TabRounding: number = 0.0;
    public TabBorderSize: number = 0.0;
    public TabMinWidthBase: number = 0.0;
    public TabMinWidthShrink: number = 0.0;
    public TabCloseButtonMinWidthSelected: number = -1.0;
    public TabCloseButtonMinWidthUnselected: number = 0.0;
    public TabBarBorderSize: number = 1.0;
    public TabBarOverlineSize: number = 2.0;
    public TableAngledHeadersAngle: number = 35.0;
    public TableAngledHeadersTextAlign: ImVec2 = new ImVec2(0.5, 0.0);
    public TreeLinesFlags: number = 0;
    public TreeLinesSize: number = 1.0;
    public TreeLinesRounding: number = 0.0;
    public DragDropTargetRounding: number = 0.0;
    public DragDropTargetBorderSize: number = 1.0;
    public DragDropTargetPadding: number = 0.0;
    public ColorMarkerSize: number = 12.0;
    ...
    public SeparatorSize: number = 1.0;
    public SeparatorTextBorderSize: number = 3.0;
    public DisplayWindowPadding: ImVec2 = new ImVec2(22, 22);
    public DisplaySafeAreaPadding: ImVec2 = new ImVec2(4, 4);
    public DockingNodeHasCloseButton: boolean = true;
    public DockingSeparatorSize: number = 2.0;
    ...
    public CircleTessellationMaxError: number = 0.30;
    public HoverStationaryDelay: number = 0.15;
    public HoverDelayShort: number = 0.15;
    public HoverDelayNormal: number = 0.40;
    public HoverFlagsForTooltipMouse: number = 0;
    public HoverFlagsForTooltipNav: number = 0;
...
export class ImGuiStyle
{
    ...
    get WindowBorderHoverPadding(): number { return this.internal.WindowBorderHoverPadding; } set WindowBorderHoverPadding(value: number) { this.internal.WindowBorderHoverPadding = value; }
    ...
    get ScrollbarPadding(): number { return this.internal.ScrollbarPadding; } set ScrollbarPadding(value: number) { this.internal.ScrollbarPadding = value; }
    ...
    get ImageRounding(): number { return this.internal.ImageRounding; } set ImageRounding(value: number) { this.internal.ImageRounding = value; }
    get ImageBorderSize(): number { return this.internal.ImageBorderSize; } set ImageBorderSize(value: number) { this.internal.ImageBorderSize = value; }
    ...
    get TabMinWidthBase(): number { return this.internal.TabMinWidthBase; } set TabMinWidthBase(value: number) { this.internal.TabMinWidthBase = value; }
    get TabMinWidthShrink(): number { return this.internal.TabMinWidthShrink; } set TabMinWidthShrink(value: number) { this.internal.TabMinWidthShrink = value; }
    get TabCloseButtonMinWidthSelected(): number { return this.internal.TabCloseButtonMinWidthSelected; } set TabCloseButtonMinWidthSelected(value: number) { this.internal.TabCloseButtonMinWidthSelected = value; }
    get TabCloseButtonMinWidthUnselected(): number { return this.internal.TabCloseButtonMinWidthUnselected; } set TabCloseButtonMinWidthUnselected(value: number) { this.internal.TabCloseButtonMinWidthUnselected = value; }
    get TabBarBorderSize(): number { return this.internal.TabBarBorderSize; } set TabBarBorderSize(value: number) { this.internal.TabBarBorderSize = value; }
    get TabBarOverlineSize(): number { return this.internal.TabBarOverlineSize; } set TabBarOverlineSize(value: number) { this.internal.TabBarOverlineSize = value; }
    get TableAngledHeadersAngle(): number { return this.internal.TableAngledHeadersAngle; } set TableAngledHeadersAngle(value: number) { this.internal.TableAngledHeadersAngle = value; }
    get TableAngledHeadersTextAlign(): Bind.interface_ImVec2 { return this.internal.TableAngledHeadersTextAlign; }
    get TreeLinesFlags(): number { return this.internal.TreeLinesFlags; } set TreeLinesFlags(value: number) { this.internal.TreeLinesFlags = value; }
    get TreeLinesSize(): number { return this.internal.TreeLinesSize; } set TreeLinesSize(value: number) { this.internal.TreeLinesSize = value; }
    get TreeLinesRounding(): number { return this.internal.TreeLinesRounding; } set TreeLinesRounding(value: number) { this.internal.TreeLinesRounding = value; }
    get DragDropTargetRounding(): number { return this.internal.DragDropTargetRounding; } set DragDropTargetRounding(value: number) { this.internal.DragDropTargetRounding = value; }
    get DragDropTargetBorderSize(): number { return this.internal.DragDropTargetBorderSize; } set DragDropTargetBorderSize(value: number) { this.internal.DragDropTargetBorderSize = value; }
    get DragDropTargetPadding(): number { return this.internal.DragDropTargetPadding; } set DragDropTargetPadding(value: number) { this.internal.DragDropTargetPadding = value; }
    get ColorMarkerSize(): number { return this.internal.ColorMarkerSize; } set ColorMarkerSize(value: number) { this.internal.ColorMarkerSize = value; }
    ...
    get SeparatorSize(): number { return this.internal.SeparatorSize; } set SeparatorSize(value: number) { this.internal.SeparatorSize = value; }
    get SeparatorTextBorderSize(): number { return this.internal.SeparatorTextBorderSize; } set SeparatorTextBorderSize(value: number) { this.internal.SeparatorTextBorderSize = value; }
    ...
    get DockingNodeHasCloseButton(): boolean { return this.internal.DockingNodeHasCloseButton; } set DockingNodeHasCloseButton(value: boolean) { this.internal.DockingNodeHasCloseButton = value; }
    get DockingSeparatorSize(): number { return this.internal.DockingSeparatorSize; } set DockingSeparatorSize(value: number) { this.internal.DockingSeparatorSize = value; }
    ...
    get HoverStationaryDelay(): number { return this.internal.HoverStationaryDelay; } set HoverStationaryDelay(value: number) { this.internal.HoverStationaryDelay = value; }
    get HoverDelayShort(): number { return this.internal.HoverDelayShort; } set HoverDelayShort(value: number) { this.internal.HoverDelayShort = value; }
    get HoverDelayNormal(): number { return this.internal.HoverDelayNormal; } set HoverDelayNormal(value: number) { this.internal.HoverDelayNormal = value; }
    get HoverFlagsForTooltipMouse(): number { return this.internal.HoverFlagsForTooltipMouse; } set HoverFlagsForTooltipMouse(value: number) { this.internal.HoverFlagsForTooltipMouse = value; }
    get HoverFlagsForTooltipNav(): number { return this.internal.HoverFlagsForTooltipNav; } set HoverFlagsForTooltipNav(value: number) { this.internal.HoverFlagsForTooltipNav = value; }
...
export class ImGuiIO
{
    ...
    get ConfigDockingNoDockingOver(): boolean { return this.native.ConfigDockingNoDockingOver; }
    set ConfigDockingNoDockingOver(value: boolean) { this.native.ConfigDockingNoDockingOver = value; }

    // Viewport options
    get ConfigViewportsNoAutoMerge(): boolean { return this.native.ConfigViewportsNoAutoMerge; }
    set ConfigViewportsNoAutoMerge(value: boolean) { this.native.ConfigViewportsNoAutoMerge = value; }
    get ConfigViewportsNoTaskBarIcon(): boolean { return this.native.ConfigViewportsNoTaskBarIcon; }
    set ConfigViewportsNoTaskBarIcon(value: boolean) { this.native.ConfigViewportsNoTaskBarIcon = value; }
    get ConfigViewportsNoDecoration(): boolean { return this.native.ConfigViewportsNoDecoration; }
    set ConfigViewportsNoDecoration(value: boolean) { this.native.ConfigViewportsNoDecoration = value; }
    get ConfigViewportsNoDefaultParent(): boolean { return this.native.ConfigViewportsNoDefaultParent; }
    set ConfigViewportsNoDefaultParent(value: boolean) { this.native.ConfigViewportsNoDefaultParent = value; }

    // Navigation options
    get ConfigNavSwapGamepadButtons(): boolean { return this.native.ConfigNavSwapGamepadButtons; }
    set ConfigNavSwapGamepadButtons(value: boolean) { this.native.ConfigNavSwapGamepadButtons = value; }
    get ConfigNavMoveSetMousePos(): boolean { return this.native.ConfigNavMoveSetMousePos; }
    set ConfigNavMoveSetMousePos(value: boolean) { this.native.ConfigNavMoveSetMousePos = value; }
    get ConfigNavCaptureKeyboard(): boolean { return this.native.ConfigNavCaptureKeyboard; }
    set ConfigNavCaptureKeyboard(value: boolean) { this.native.ConfigNavCaptureKeyboard = value; }
    get ConfigNavEscapeClearFocusItem(): boolean { return this.native.ConfigNavEscapeClearFocusItem; }
    set ConfigNavEscapeClearFocusItem(value: boolean) { this.native.ConfigNavEscapeClearFocusItem = value; }
    get ConfigNavEscapeClearFocusWindow(): boolean { return this.native.ConfigNavEscapeClearFocusWindow; }
    set ConfigNavEscapeClearFocusWindow(value: boolean) { this.native.ConfigNavEscapeClearFocusWindow = value; }
    get ConfigNavCursorVisibleAuto(): boolean { return this.native.ConfigNavCursorVisibleAuto; }
    set ConfigNavCursorVisibleAuto(value: boolean) { this.native.ConfigNavCursorVisibleAuto = value; }
    get ConfigNavCursorVisibleAlways(): boolean { return this.native.ConfigNavCursorVisibleAlways; }
    set ConfigNavCursorVisibleAlways(value: boolean) { this.native.ConfigNavCursorVisibleAlways = value; }

    // DPI/Scaling options
    get ConfigDpiScaleFonts(): boolean { return this.native.ConfigDpiScaleFonts; }
    set ConfigDpiScaleFonts(value: boolean) { this.native.ConfigDpiScaleFonts = value; }
    get ConfigDpiScaleViewports(): boolean { return this.native.ConfigDpiScaleViewports; }
    set ConfigDpiScaleViewports(value: boolean) { this.native.ConfigDpiScaleViewports = value; }
    ...
    get ConfigInputTrickleEventQueue(): boolean { return this.native.ConfigInputTrickleEventQueue; }
    set ConfigInputTrickleEventQueue(value: boolean) { this.native.ConfigInputTrickleEventQueue = value; }
    ...
    get ConfigInputTextEnterKeepActive(): boolean { return this.native.ConfigInputTextEnterKeepActive; }
    set ConfigInputTextEnterKeepActive(value: boolean) { this.native.ConfigInputTextEnterKeepActive = value; }
    ...
    get ConfigWindowsCopyContentsWithCtrlC(): boolean { return this.native.ConfigWindowsCopyContentsWithCtrlC; }
    set ConfigWindowsCopyContentsWithCtrlC(value: boolean) { this.native.ConfigWindowsCopyContentsWithCtrlC = value; }
    get ConfigScrollbarScrollByPage(): boolean { return this.native.ConfigScrollbarScrollByPage; }
    set ConfigScrollbarScrollByPage(value: boolean) { this.native.ConfigScrollbarScrollByPage = value; }
    ...
    // Debug options
    get ConfigErrorRecovery(): boolean { return this.native.ConfigErrorRecovery; }
    set ConfigErrorRecovery(value: boolean) { this.native.ConfigErrorRecovery = value; }
    get ConfigDebugIsDebuggerPresent(): boolean { return this.native.ConfigDebugIsDebuggerPresent; }
    set ConfigDebugIsDebuggerPresent(value: boolean) { this.native.ConfigDebugIsDebuggerPresent = value; }
    get ConfigDebugHighlightIdConflicts(): boolean { return this.native.ConfigDebugHighlightIdConflicts; }
    set ConfigDebugHighlightIdConflicts(value: boolean) { this.native.ConfigDebugHighlightIdConflicts = value; }
    ...
    // Input Functions
    public AddKeyEvent(key: ImGuiKey, down: boolean): void { this.native.AddKeyEvent(key, down); }
    public AddKeyAnalogEvent(key: ImGuiKey, down: boolean, v: number): void { this.native.AddKeyAnalogEvent(key, down, v); }
    public AddMousePosEvent(x: number, y: number): void { this.native.AddMousePosEvent(x, y); }
    public AddMouseButtonEvent(button: number, down: boolean): void { this.native.AddMouseButtonEvent(button, down); }
    public AddMouseWheelEvent(wheel_x: number, wheel_y: number): void { this.native.AddMouseWheelEvent(wheel_x, wheel_y); }
    public AddMouseSourceEvent(source: number): void { this.native.AddMouseSourceEvent(source); }
    public AddFocusEvent(focused: boolean): void { this.native.AddFocusEvent(focused); }
    ...
    public SetKeyEventNativeData(key: ImGuiKey, native_keycode: number, native_scancode: number, native_legacy_index: number = -1): void { this.native.SetKeyEventNativeData(key, native_keycode, native_scancode, native_legacy_index); }
    public SetAppAcceptingEvents(accepting_events: boolean): void { this.native.SetAppAcceptingEvents(accepting_events); }
    public ClearEventsQueue(): void { this.native.ClearEventsQueue(); }
    public ClearInputKeys(): void { this.native.ClearInputKeys(); }
    public ClearInputMouse(): void { this.native.ClearInputMouse(); }
    ...
    public MouseClickedCount: number[] = new Proxy([], {
        get: (target: number[], key: PropertyKey): number => {
            if (key === "length") { return 5; }
...
export function ShowDebugLogWindow(p_open: Bind.ImScalar<boolean> | Bind.ImAccess<boolean> | null = null): void {
    if (p_open === null) {
...
export function ShowIDStackToolWindow(p_open: Bind.ImScalar<boolean> | Bind.ImAccess<boolean> | null = null): void {
    if (p_open === null) {
...
export function BeginChild(id: string | ImGuiID, size: Readonly<Bind.interface_ImVec2> = ImVec2.ZERO, child_flags: ImGuiChildFlags = 0, window_flags: ImGuiWindowFlags = 0): boolean {
...
export function GetWindowDpiScale(): number { return bind.GetWindowDpiScale(); }
export function GetWindowViewport(): ImGuiViewport | null {
...
export function PushFont(font: ImFont | null, font_size_base_unscaled?: number): void {
...
export function PushStyleVarX(idx: ImGuiStyleVar, val_x: number): void { bind.PushStyleVarX(idx, val_x); }
export function PushStyleVarY(idx: ImGuiStyleVar, val_y: number): void { bind.PushStyleVarY(idx, val_y); }
...
export function PushItemFlag(flag: number, enabled: boolean): void { bind.PushItemFlag(flag, enabled); }
export function PopItemFlag(): void { bind.PopItemFlag(); }
...
export function GetFontBaked(): ImFontBaked | null {
...
export function SeparatorText(label: string): void { bind.SeparatorText(label); }
...
export function TextLink(label: string): boolean { return bind.TextLink(label); }
export function TextLinkOpenURL(label: string, url: string = ""): void { bind.TextLinkOpenURL(label, url); }
...
export function Image(user_texture_id: ImTextureID | null, size: Readonly<Bind.interface_ImVec2>, uv0: Readonly<Bind.interface_ImVec2> = ImVec2.ZERO, uv1: Readonly<Bind.interface_ImVec2> = ImVec2.UNIT): void {
    bind.Image(ImGuiContext.setTexture(user_texture_id), size, uv0, uv1);
}
export function ImageWithBg(user_texture_id: ImTextureID | null, size: Readonly<Bind.interface_ImVec2>, uv0: Readonly<Bind.interface_ImVec2> = ImVec2.ZERO, uv1: Readonly<Bind.interface_ImVec2> = ImVec2.UNIT, bg_col: Readonly<Bind.interface_ImVec4> = ImVec4.ZERO, tint_col: Readonly<Bind.interface_ImVec4> = ImVec4.WHITE): void {
    bind.ImageWithBg(ImGuiContext.setTexture(user_texture_id), size, uv0, uv1, bg_col, tint_col);
}
export function ImageButton(str_id: string, user_texture_id: ImTextureID | null, size: Readonly<Bind.interface_ImVec2>, uv0: Readonly<Bind.interface_ImVec2> = ImVec2.ZERO, uv1: Readonly<Bind.interface_ImVec2> = ImVec2.UNIT, bg_col: Readonly<Bind.interface_ImVec4> = ImVec4.ZERO, tint_col: Readonly<Bind.interface_ImVec4> = ImVec4.WHITE): boolean {
...
export function Combo<T>(label: string, current_item: Bind.ImAccess<number> | Bind.ImScalar<number>, ...args: any[]): boolean {
    ...
    if (Array.isArray(args[0])) {
        ...
        const items_getter = (_: null, idx: number): string => items[idx];
        ...
        const items_getter: ComboValueGetter<T> = args[0];
        const data: T = args[1];
...
export function SetNextItemStorageID(storage_id: ImGuiID): void {
    bind.SetNextItemStorageID(storage_id);
}
export function TreeNodeGetOpen(storage_id: ImGuiID): boolean {
    return bind.TreeNodeGetOpen(storage_id);
}
export function TreeNodeSetOpen(storage_id: ImGuiID, open: boolean): void {
...
export function BeginMultiSelect(flags: ImGuiMultiSelectFlags = 0, selection_size: number = -1, items_count: number = -1): ImGuiMultiSelectIO | null {
    const ms_io = bind.BeginMultiSelect(flags, selection_size, items_count);
    return ms_io === null ? null : new ImGuiMultiSelectIO(ms_io);
}
export function EndMultiSelect(): ImGuiMultiSelectIO | null {
    const ms_io = bind.EndMultiSelect();
    return ms_io === null ? null : new ImGuiMultiSelectIO(ms_io);
}
export function SetNextItemSelectionUserData(selection_user_data: ImGuiSelectionUserData): void { bind.SetNextItemSelectionUserData(selection_user_data); }
export function IsItemToggledSelection(): boolean { return bind.IsItemToggledSelection(); }
...
export function BeginListBox(label: string, size: Readonly<Bind.interface_ImVec2> = ImVec2.ZERO): boolean {
...
export function BeginItemTooltip(): boolean { return bind.BeginItemTooltip(); }
export function SetItemTooltip(fmt: string): void { bind.SetItemTooltip(fmt); }
...
export function TableAngledHeadersRow(): void { bind.TableAngledHeadersRow(); }
...
export function TableSetColumnEnabled(column_n: number, enabled: boolean): void { bind.TableSetColumnEnabled(column_n, enabled); }
export function TableGetHoveredColumn(): number { return bind.TableGetHoveredColumn(); }
...
export function DockSpaceOverViewport(): Bind.ImGuiID;
export function DockSpaceOverViewport(viewport: ImGuiViewport | null, flags?: ImGuiDockNodeFlags, window_class?: Readonly<Bind.interface_ImGuiWindowClass> | null): Bind.ImGuiID;
export function DockSpaceOverViewport(dockspace_id: Bind.ImGuiID, viewport?: ImGuiViewport | null, flags?: ImGuiDockNodeFlags, window_class?: Readonly<Bind.interface_ImGuiWindowClass> | null): Bind.ImGuiID;
export function DockSpaceOverViewport(...args: any[]): Bind.ImGuiID {
    if (args.length === 0) {
        return bind.DockSpaceOverViewport(0, null, 0, null);
    }

    if (args[0] instanceof ImGuiViewport || args[0] === null) {
        const viewport: ImGuiViewport | null = args[0];
        const flags: ImGuiDockNodeFlags = typeof args[1] === "number" ? args[1] : 0;
        const window_class: Readonly<Bind.interface_ImGuiWindowClass> | null = args[2] || null;
        return bind.DockSpaceOverViewport(0, viewport ? viewport.native : null, flags, window_class);
    }

    const dockspace_id: Bind.ImGuiID = args[0];
    const viewport: ImGuiViewport | null = args[1] instanceof ImGuiViewport ? args[1] : null;
    const flags: ImGuiDockNodeFlags = typeof args[1] === "number" ? args[1] : (typeof args[2] === "number" ? args[2] : 0);
    const window_class: Readonly<Bind.interface_ImGuiWindowClass> | null = args[1] instanceof ImGuiViewport ? (args[3] || null) : (args[2] || null);
...
export function SetNextItemAllowOverlap(): void { bind.SetNextItemAllowOverlap(); }
...
export function SetStateStorage(storage: ImGuiStorage | null): void { bind.SetStateStorage(storage ? storage.native : null); }
export function GetStateStorage(): ImGuiStorage | null {
...
export function IsKeyDown(key: ImGuiKey): boolean { return bind.IsKeyDown(key); }
export function IsKeyPressed(key: ImGuiKey, repeat: boolean = true): boolean { return bind.IsKeyPressed(key, repeat); }
export function IsKeyReleased(key: ImGuiKey): boolean { return bind.IsKeyReleased(key); }
export function IsKeyChordPressed(key_chord: ImGuiKeyChord): boolean { return bind.IsKeyChordPressed(key_chord); }
export function GetKeyPressedAmount(key: ImGuiKey, repeat_delay: number, rate: number): number { return bind.GetKeyPressedAmount(key, repeat_delay, rate); }
export function GetKeyName(key: ImGuiKey): string { return bind.GetKeyName(key); }
export function SetNextFrameWantCaptureKeyboard(capture: boolean): void { bind.SetNextFrameWantCaptureKeyboard(capture); }
export function Shortcut(key_chord: ImGuiKeyChord, flags: ImGuiInputFlags = 0): boolean { return bind.Shortcut(key_chord, flags); }
export function SetNextItemShortcut(key_chord: ImGuiKeyChord, flags: ImGuiInputFlags = 0): void { bind.SetNextItemShortcut(key_chord, flags); }
export function SetItemKeyOwner(key: ImGuiKey): void { bind.SetItemKeyOwner(key); }
...
export function SetNextFrameWantCaptureMouse(capture: boolean): void { bind.SetNextFrameWantCaptureMouse(capture); }
...
export function LoadIniSettingsFromDisk(ini_filename: string): void { bind.LoadIniSettingsFromDisk(ini_filename); }
export function LoadIniSettingsFromMemory(ini_data: string, ini_size: number = 0): void { bind.LoadIniSettingsFromMemory(ini_data); }
export function SaveIniSettingsToDisk(ini_filename: string): void { bind.SaveIniSettingsToDisk(ini_filename); }
...
export function DebugStartItemPicker(): void { bind.DebugStartItemPicker(); }
export function DebugFlashStyleColor(idx: ImGuiCol): void { bind.DebugFlashStyleColor(idx); }
export function DebugTextEncoding(text: string): void { bind.DebugTextEncoding(text); }
...
export function GetPlatformIO(): ImGuiPlatformIO { return new ImGuiPlatformIO(bind.GetPlatformIO()); }
...
export function UpdatePlatformWindows(): void { bind.UpdatePlatformWindows(); }
export function RenderPlatformWindowsDefault(platform_render_arg: number | null = null, renderer_render_arg: number | null = null): void {
    bind.RenderPlatformWindowsDefault(platform_render_arg, renderer_render_arg);
}
export function DestroyPlatformWindows(): void { bind.DestroyPlatformWindows(); }
export function FindViewportByID(id: ImGuiID): ImGuiViewport | null {
    const viewport = bind.FindViewportByID(id);
    return viewport === null ? null : new ImGuiViewport(viewport);
}
export function FindViewportByPlatformHandle(platform_handle: number | null): ImGuiViewport | null {
