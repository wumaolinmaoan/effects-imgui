...
type ImDrawCornerFlags = number;      // -> enum ImDrawCornerFlags_    // [Deprecated] Use ImDrawFlags
type ImDrawFlags = number;            // -> enum ImDrawFlags_          // Flags: for ImDrawList functions
...
type ImGuiInputFlags = number;        // -> enum ImGuiInputFlags_     // Flags: for Shortcut(), SetNextItemShortcut()
type ImGuiInputTextFlags = number;    // -> enum ImGuiInputTextFlags_  // Flags: for InputText(), InputTextMultiline()
type ImGuiKeyChord = number;          // -> ImGuiKey | ImGuiMod_XXX    // Flags: for Shortcut(), IsKeyChordPressed()
type ImGuiKeyModFlags = number;       // -> enum ImGuiKeyModFlags_     // Flags: for io.KeyMods (Ctrl/Shift/Alt/Super)
type ImGuiMultiSelectFlags = number;  // -> enum ImGuiMultiSelectFlags_// Flags: for BeginMultiSelect()
...
type ImGuiChildFlags = number;        // -> enum ImGuiChildFlags_      // Flags: for BeginChild()
...
export type ImGuiSelectionUserData = number;
...
export interface reference_ImGuiStorage extends Emscripten.EmscriptenClassReference {
    Clear(): void;
...
export class ImGuiStorage extends Emscripten.EmscriptenClass implements reference_ImGuiStorage {
    public Clear(): void;
    public GetInt(key: ImGuiID, default_val: number): number;
    public SetInt(key: ImGuiID, val: number): void;
    public GetBool(key: ImGuiID, default_val: boolean): boolean;
    public SetBool(key: ImGuiID, val: boolean): void;
    public GetFloat(key: ImGuiID, default_val: number): number;
    public SetFloat(key: ImGuiID, val: number): void;
    public GetVoidPtr(key: ImGuiID): number | null;
    public SetVoidPtr(key: ImGuiID, val: number | null): void;
...
export interface reference_ImGuiViewport extends Emscripten.EmscriptenClassReference {
    ...
    GetCenter(out?: interface_ImVec2): interface_ImVec2;
    GetWorkCenter(out?: interface_ImVec2): interface_ImVec2;

    //ImGuiTableSortSpecs()       { memset(this, 0, sizeof(*this)); }
}

export interface reference_ImGuiPlatformMonitor extends Emscripten.EmscriptenClassReference {
    MainPos: reference_ImVec2;
    MainSize: reference_ImVec2;
    WorkPos: reference_ImVec2;
    WorkSize: reference_ImVec2;
    DpiScale: number;
    PlatformHandle: number | null;
}

export interface interface_ImGuiPlatformMonitor {
    MainPos: reference_ImVec2;
    MainSize: reference_ImVec2;
    WorkPos: reference_ImVec2;
    WorkSize: reference_ImVec2;
...
export interface reference_ImGuiPlatformIO extends Emscripten.EmscriptenClassReference {
    IterateMonitors(callback: (monitor: interface_ImGuiPlatformMonitor) => void): void;
    IterateViewports(callback: (viewport: reference_ImGuiViewport) => void): void;
    Platform_LocaleDecimalPoint: number;
    Renderer_TextureMaxWidth: number;
    Renderer_TextureMaxHeight: number;
    readonly Renderer_RenderState: number | null;
    ClearPlatformHandlers(): void;
    ClearRendererHandlers(): void;
}

export interface interface_ImGuiSelectionRequest {
    Type: number;
    Selected: boolean;
...
export interface reference_ImGuiMultiSelectIO extends Emscripten.EmscriptenClassReference {
    _getRequests(): interface_ImGuiSelectionRequest[];
...
export interface interface_ImGuiSelectionIteratorResult {
    done: boolean;
    id: ImGuiID;
    it: number | null;
}

export class ImGuiSelectionBasicStorage extends Emscripten.EmscriptenClass {
    Size: number;
    PreserveOrder: boolean;
    ApplyRequests(ms_io: reference_ImGuiMultiSelectIO): void;
    Contains(id: ImGuiID): boolean;
    Clear(): void;
    Swap(r: ImGuiSelectionBasicStorage): void;
    SetItemSelected(id: ImGuiID, selected: boolean): void;
    GetNextSelectedItem(opaque_it: number | null): interface_ImGuiSelectionIteratorResult;
    GetStorageIdFromIndex(idx: number): ImGuiID;
    SetAdapterIndexToStorageId(callback: null): void;
}
...
export interface interface_ImGuiStyle {
    ...
    WindowBorderHoverPadding: number;
    ...
    TabCloseButtonMinWidthSelected: number;
    TabCloseButtonMinWidthUnselected: number;
    TabBarBorderSize: number;
    TabBarOverlineSize: number;
    TableAngledHeadersAngle: number;
    readonly TableAngledHeadersTextAlign: interface_ImVec2;
    TreeLinesFlags: number;
    TreeLinesSize: number;
    TreeLinesRounding: number;
    DragDropTargetRounding: number;
    DragDropTargetBorderSize: number;
    DragDropTargetPadding: number;
    ...
    HoverFlagsForTooltipMouse: number;
    HoverFlagsForTooltipNav: number;
...
export class ImGuiStyle extends Emscripten.EmscriptenClass implements interface_ImGuiStyle {
    ...
    TabBarOverlineSize: number;
    TableAngledHeadersAngle: number;
    readonly TableAngledHeadersTextAlign: reference_ImVec2;
    TreeLinesFlags: number;
    TreeLinesSize: number;
    TreeLinesRounding: number;
    DragDropTargetRounding: number;
...
export interface reference_ImDrawList extends Emscripten.EmscriptenClassReference {
    ...
    AddConcavePolyFilled(points: Readonly<interface_ImVec2>[], num_points: number, col: ImU32): void;
    ...
    AddEllipse(center: Readonly<interface_ImVec2>, radius: Readonly<interface_ImVec2>, col: ImU32, rot: number, num_segments: number, thickness: number): void;
    // IMGUI_API void  AddEllipseFilled(const ImVec2& center, const ImVec2& radius, ImU32 col, float rot = 0.0f, int num_segments = 0);
    AddEllipseFilled(center: Readonly<interface_ImVec2>, radius: Readonly<interface_ImVec2>, col: ImU32, rot: number, num_segments: number): void;
    ...
    // inline    void  PathFillConcave(ImU32 col)                                 { AddConcavePolyFilled(_Path.Data, _Path.Size, col); PathClear(); }
    ...
    PathEllipticalArcTo(center: Readonly<interface_ImVec2>, radius: Readonly<interface_ImVec2>, rot: number, a_min: number, a_max: number, num_segments: number): void;
    ...
    // IMGUI_API void  Clear();                    // Not bound in cpp
    // Clear(): void;
    // IMGUI_API void  ClearFreeMemory();           // Not bound in cpp
    ...
    // IMGUI_API void  UpdateClipRect();             // Not bound in cpp
    // UpdateClipRect(): void;
    // IMGUI_API void  UpdateTextureID();             // Not bound in cpp
...
export interface reference_ImFontBaked extends Emscripten.EmscriptenClassReference {
    IterateGlyphs(callback: (glyph: interface_ImFontGlyph) => void): void;
    ...
    Ascent: number;
    Descent: number;
    readonly OwnerFont: reference_ImFont | null;
...
export interface reference_ImFont extends Emscripten.EmscriptenClassReference {
    ...
    FontId: number;
    // float                       LegacySize;         // Font size passed to AddFont(). Use for old code calling PushFont() expecting to use that size.
    LegacySize: number;

    // ImVector<ImFontGlyph>       Glyphs;             //              // All glyphs.
    IterateGlyphs(callback: (cfg: interface_ImFontGlyph) => void): void;
    ...
    GetFontBaked(font_size: number, density: number): reference_ImFontBaked | null;
    ...
    RenderText(draw_list: reference_ImDrawList, size: number, pos: Readonly<interface_ImVec2>, col: ImU32, clip_rect: Readonly<interface_ImVec4>, text_begin: string, wrap_width: number, flags: ImDrawFlags): void;
...
export interface interface_ImFontConfig {
    ...
    // const ImWchar*  GlyphRanges;                // NULL     // Pointer to a user-provided list of Unicode range
    GlyphRanges: number | null;
    // float           GlyphMinAdvanceX;           // 0        // Minimum AdvanceX for glyphs
    GlyphMinAdvanceX: number;
    // float           GlyphMaxAdvanceX;           // FLT_MAX  // Maximum AdvanceX for glyphs
    GlyphMaxAdvanceX: number;
    // bool            MergeMode;                  // false    // Merge into previous ImFont
    MergeMode: boolean;
    // unsigned int    FontLoaderFlags;            // 0x00     // Settings for custom font loader/rasterizer
    FontLoaderFlags: number;
    // float           RasterizerMultiply;         // 1.0f     // Brighten (>1.0f) or darken (<1.0f) font output.
    RasterizerMultiply: number;
    // float           RasterizerDensity;          // 1.0f     // DPI scale for rasterization
    RasterizerDensity: number;
...
export interface interface_ImFontAtlasRect {
    x: number;
    y: number;
    w: number;
    h: number;
    uv0: Readonly<interface_ImVec2>;
    uv1: Readonly<interface_ImVec2>;
}
...
export interface reference_ImFontAtlas extends Emscripten.EmscriptenClassReference {
    ...
    AddFont(font_cfg: interface_ImFontConfig): reference_ImFont;
    // IMGUI_API ImFont*           AddFontDefault(const ImFontConfig* font_cfg = NULL);
    AddFontDefault(font_cfg: interface_ImFontConfig | null): reference_ImFont;
    AddFontDefaultVector(font_cfg: interface_ImFontConfig | null): reference_ImFont;
    AddFontDefaultBitmap(font_cfg: interface_ImFontConfig | null): reference_ImFont;
    // IMGUI_API ImFont*           AddFontFromFileTTF(const char* filename, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);
    AddFontFromFileTTF(filename: string, size_pixels: number, font_cfg: interface_ImFontConfig | null, glyph_ranges: number | null): reference_ImFont;
    ...
    AddFontFromMemoryCompressedTTF(data: Uint8Array, size_pixels: number, font_cfg: interface_ImFontConfig | null, glyph_ranges: number | null): reference_ImFont;
    // IMGUI_API ImFont*           AddFontFromMemoryCompressedBase85TTF(const char* compressed_font_data_base85, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);              // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
    AddFontFromMemoryCompressedBase85TTF(data: string, size_pixels: number, font_cfg: interface_ImFontConfig | null, glyph_ranges: number | null): reference_ImFont;
    RemoveFont(font: reference_ImFont | null): void;
    ...
    GetCustomRect(id: ImFontAtlasRectId): interface_ImFontAtlasRect | null;
    AddCustomRectFontGlyph(font: reference_ImFont | null, codepoint: ImWchar, w: number, h: number, advance_x: number, offset: Readonly<interface_ImVec2>): ImFontAtlasRectId;
    AddCustomRectFontGlyphForSize(font: reference_ImFont | null, font_size: number, codepoint: ImWchar, w: number, h: number, advance_x: number, offset: Readonly<interface_ImVec2>): ImFontAtlasRectId;
    ...
    TexID: ImTextureID;
    // int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. (removed in v1.92)
...
export interface reference_ImGuiIO extends Emscripten.EmscriptenClassReference {
    ...
    ConfigDockingNoDockingOver: boolean;
    ...
    _getAt_MouseClickedCount(index: number): number;
...
export interface Module extends Emscripten.EmscriptenModule {
    ...
    ImGuiStorage: { new(): ImGuiStorage; };
    ImGuiStyle: { new(): ImGuiStyle; };
    ImGuiSelectionBasicStorage: { new(): ImGuiSelectionBasicStorage; };
    ...
    ShowDebugLogWindow(p_open: ImScalar<boolean> | null): void;
    ShowIDStackToolWindow(p_open: ImScalar<boolean> | null): void;
    ...
    BeginChild(id: string | ImGuiID, size: Readonly<interface_ImVec2>, child_flags: ImGuiChildFlags, window_flags: ImGuiWindowFlags): boolean;
    EndChild(): void;
    ...
    GetWindowViewport(): reference_ImGuiViewport | null;
    ...
    SetNextWindowScroll(scroll: Readonly<interface_ImVec2>): void;
    ...
    PushFont(font: reference_ImFont | null, font_size_base_unscaled: number): void;
    PopFont(): void;
    ...
    PopTextWrapPos(): void;
    PushItemFlag(flag: number, enabled: boolean): void;
    PopItemFlag(): void;
    ...
    GetFontSize(): number;
    GetFontBaked(): reference_ImFontBaked | null;
    ...
    Image(user_texture_id: any, size: Readonly<interface_ImVec2>, uv0: Readonly<interface_ImVec2>, uv1: Readonly<interface_ImVec2>): void;
    ImageWithBg(user_texture_id: any, size: Readonly<interface_ImVec2>, uv0: Readonly<interface_ImVec2>, uv1: Readonly<interface_ImVec2>, bg_col: Readonly<interface_ImVec4>, tint_col: Readonly<interface_ImVec4>): void;
    ImageButton(str_id: string, user_texture_id: any, size: Readonly<interface_ImVec2>, uv0: Readonly<interface_ImVec2>, uv1: Readonly<interface_ImVec2>, bg_col: Readonly<interface_ImVec4>, tint_col: Readonly<interface_ImVec4>): boolean;
    ...
    EndCombo(): void;
    Combo<T>(label: string, current_item: ImScalar<number>, items_getter: (data: T, idx: number) => string, data: T, items_count: number, popup_max_height_in_items: number): boolean;
    ...
    TreeNodeSetOpen(storage_id: ImGuiID, open: boolean): void;
    ...
    BeginMultiSelect(flags: ImGuiMultiSelectFlags, selection_size: number, items_count: number): reference_ImGuiMultiSelectIO | null;
    EndMultiSelect(): reference_ImGuiMultiSelectIO | null;
    ...
    ListBox_B<T>(label: string, current_item: ImScalar<number>, items_getter: (data: T, idx: number) => string, data: T, items_count: number, height_in_items: number): boolean;
    ...
    TableSetColumnEnabled(column_n: number, enabled: boolean): void;
    ...
    DockSpaceOverViewport(dockspace_id: ImGuiID, viewport: reference_ImGuiViewport | null, flags: ImGuiDockNodeFlags, window_class: Readonly<interface_ImGuiWindowClass> | null): ImGuiID;
    ...
    SetDragDropPayload(type: string, data: any, cond: ImGuiCond): boolean;
    EndDragDropSource(): void;
    BeginDragDropTarget(): boolean;
    AcceptDragDropPayload(type: string, flags: ImGuiDragDropFlags): any | null;
    EndDragDropTarget(): void;
    GetDragDropPayload(): any | null;
    ...
    SetStateStorage(tree: reference_ImGuiStorage | null): void;
    GetStateStorage(): reference_ImGuiStorage | null;
    ...
    DebugFlashStyleColor(idx: number): void;
    ...
    GetPlatformIO(): reference_ImGuiPlatformIO;
    ...
    RenderPlatformWindowsDefault(platform_render_arg: number | null, renderer_render_arg: number | null): void;
    DestroyPlatformWindows(): void;
    FindViewportByID(id: ImGuiID): reference_ImGuiViewport | null;
    FindViewportByPlatformHandle(platform_handle: number | null): reference_ImGuiViewport | null;
}
