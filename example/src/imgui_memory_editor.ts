...
export class MemoryEditor
{
    ...
    public DrawContents(mem_data: ArrayBuffer, mem_size: number = mem_data.byteLength, base_display_addr: number = 0x0000): void
    {
        ...
        ImGui.BeginChild("##scrolling", new ImGui.Vec2(0, -footer_height), ImGui.ChildFlags.None, ImGui.WindowFlags.NoMove | ImGui.WindowFlags.NoNav);
        ...
        if (this.DataEditingAddr !== <size_t>-1)
        {
            ...
            if (ImGui.IsKeyPressed(ImGui.Key.UpArrow) && this.DataEditingAddr >= <size_t>this.Cols)          { data_editing_addr_next = this.DataEditingAddr - this.Cols; this.DataEditingTakeFocus = true; }
            else if (ImGui.IsKeyPressed(ImGui.Key.DownArrow) && this.DataEditingAddr < mem_size - this.Cols) { data_editing_addr_next = this.DataEditingAddr + this.Cols; this.DataEditingTakeFocus = true; }
            else if (ImGui.IsKeyPressed(ImGui.Key.LeftArrow) && this.DataEditingAddr > 0)               { data_editing_addr_next = this.DataEditingAddr - 1; this.DataEditingTakeFocus = true; }
            else if (ImGui.IsKeyPressed(ImGui.Key.RightArrow) && this.DataEditingAddr < mem_size - 1)   { data_editing_addr_next = this.DataEditingAddr + 1; this.DataEditingTakeFocus = true; }
        }
